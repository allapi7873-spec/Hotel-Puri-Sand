...
new_nav = """<!-- Mobile Only Bottom Navigation Bar -->
  <div class="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#050508] backdrop-blur-md border-t border-gold/20 shadow-2xl flex justify-around items-center py-2 px-1">
    ...
    <!-- Floating App Food Menu Button (Same as index.html for consistency) -->
    ...
    <!-- Cart Button (Instead of WhatsApp on food page) -->
    <button id="open-cart-btn-mobile" class="mobile-tab-btn flex flex-col items-center justify-center text-[10px] text-gray-400 flex-1 py-1" onclick="toggleCart()">
      <div class="relative">
        <i class="fa-solid fa-cart-shopping text-lg mb-0.5" style="font-size: 1.1rem; margin-bottom: 2px;"></i>
        <span id="mobile-cart-badge" class="absolute -top-1.5 -right-2 bg-gold text-[#050508] rounded-full px-1 text-[10px] font-bold">0</span>
      </div>
...
if old_nav_regex.search(html):
    ...
    print("Could not find the navigation block in food.html!")

with open("food.html", "w", encoding="utf-8") as f:
    f.write(html)

print("Updated food.html")
